...
Raspberry~Pi~5 drone companion computer and a Windows~10 GCS.
...
(\num{19600}~timed operations) and validate 71 of 72~suites through
complete end-to-end tunnel runs on real hardware.  KEM keygen times
span \emph{four orders of magnitude}---from \SI{0.08}{\milli\second}
(ML-KEM-512) to \SI{8835}{\milli\second} (Classic~McEliece-8192128)
...
rekey overhead below 0.03\% at 60-second intervals, while
Classic~McEliece tunnels incur up to 13.3\% overhead---a 440$\times$
...
\begin{enumerate}
  \item \textbf{Extreme performance heterogeneity.}
    ...
    from \SI{0.08}{\milli\second} to \SI{8835}{\milli\second}---a
    ratio exceeding $10^5$.  Choosing the wrong algorithm can cause
...
\begin{enumerate}[label=\textbf{C\arabic*}:]
  ...
  \item We benchmark \textbf{\num{19600} cryptographic operations}
    and \textbf{71 end-to-end tunnel handshakes} on a Raspberry~Pi~5,
...
This reduces handshake time by \textbf{635$\times$} (from
\SI{9528}{\milli\second} to \SI{15}{\milli\second}) and public-key
...
Each cryptographic primitive was benchmarked with $n = 200$ iterations
(\num{19600}~total operations across 9~KEMs~$\times$~3~ops, 8~SIGs
~$\times$~3~ops, 3~AEADs~$\times$~2~ops~$\times$~4~payload sizes).
...
\midrule
HQC-128 (L1)          & 22.1  & 44.7  & 73.0  & \num{2249} \\
HQC-192 (L3)          & 67.4  & 135.4 & 211.2 & \num{4522} \\
HQC-256 (L5)          & 123.6 & 248.8 & 392.3 & \num{7245} \\
\midrule
...
\begin{figure}[t]
  ...
  \caption{KEM keygen time distribution (log scale).  ML-KEM
    ...
    Classic~McEliece-8192128 keygen takes up to \SI{36.6}{\second}
    (worst case), spanning over four orders of magnitude.}
  \label{fig:kem-keygen}
\end{figure}

\noindent\textbf{Key finding 1:}
ML-KEM operations complete in $< \SI{0.15}{\milli\second}$ (medians:
\SI{0.08}{\milli\second}--\SI{0.14}{\milli\second}), three orders of
magnitude faster than HQC (\SI{22}{\milli\second}--\SI{392}{\milli\second})
and five orders faster than McEliece keygen
(\SI{333}{\milli\second}--\SI{8835}{\milli\second}).

\noindent\textbf{Key finding 2:}
McEliece keygen exhibits extreme variance: the standard deviation for
McEliece-8192128 is \SI{6920}{\second}---nearly matching the
mean---due to probabilistic key generation.  This unpredictability
makes it especially unsuitable for time-bounded rekey operations.
...
combination (\SI{0.65}{\milli\second}/\SI{0.11}{\milli\second} for
Falcon-512), followed by ML-DSA.
SPHINCS\textsuperscript{+} signing exceeds \SI{1}{\second} at all
...
\begin{table}[t]
\centering
\caption{AEAD performance for 64-byte MAVLink payloads (median, $n\!=\!200$)}
...
\midrule
AES-256-GCM       & \SI{7.3}{\micro\second}  & \SI{7.7}{\micro\second}  & 1.00$\times$ \\
ChaCha20-Poly1305 & \SI{6.7}{\micro\second}  & \SI{7.1}{\micro\second}  & 0.93$\times$ \\
Ascon-128a         & \SI{4.1}{\micro\second}  & \SI{4.2}{\micro\second}  & \textbf{0.56}$\times$ \\
\bottomrule
\end{tabular}
\end{table}

Ascon-128a is 44\% faster than AES-256-GCM on the ARM~Cortex-A76,
which lacks AES-NI but benefits from efficient bitsliced Ascon
implementations.  At \SI{320}{\hertz} MAVLink rate, per-packet AEAD
overhead is $< \SI{8}{\micro\second}$---negligible relative to the
\SI{3.125}{\milli\second} inter-packet interval (0.25\% of packet
budget).
...
\SI{3.5}{\micro\second}/packet, translating to
\SI{1.12}{\milli\second\per\second} at \SI{320}{\hertz}---invisible
in practice.  The rekey decision should therefore be driven entirely
...
The mean handshake time increases 10.8$\times$ from L1 to L5, but
this is dominated by the McEliece and SPHINCS\textsuperscript{+}
components.  ML-KEM suites at all three levels complete in
$< \SI{20}{\milli\second}$.

\noindent\textbf{Regression model} (from \num{4600} measurements):
\begin{equation}
  \log_{10}(\Ths) = -1.42 + 0.87\,\log_{10}(\text{pk\_size})
  + 0.31\,\log_{10}(\text{sig\_size})
  \label{eq:regression}
\end{equation}
with $R^2 = 0.96$.  Public-key size is the dominant predictor of
handshake latency, explaining 87\% of the variance.
...
ML-KEM-768 + ML-DSA-65     & L3 & 14.8  & \num{1184} \\
ML-KEM-1024 + Falcon-1024  & L5 & 11.0  & \num{1568} \\
\bottomrule
\end{tabular}
\end{table}
...
handshake is so fast that the SIG choice (Falcon vs.\ ML-DSA)
...
\midrule
ML-KEM-768       & \SI{4.1}{\milli\second}  & 0.007\% & 0.001\% & ${<}0.001\%$ \\
HQC-256          & \SI{96}{\milli\second}   & 0.16\%  & 0.032\% & 0.003\% \\
McE-348864       & \SI{287}{\milli\second}  & 0.48\%  & 0.096\% & 0.008\% \\
McE-8192128      & \SI{9.2}{\second}        & \textbf{13.3\%} & \textbf{2.97\%} & 0.25\% \\
\bottomrule
\end{tabular}
\end{table}
...
($\PhiR < 0.01\%$), while McEliece-8192128 consumes 13.3\% of the
cycle in handshake---a \textbf{1900$\times$} difference.  This
confirms that runtime cipher-suite management is not a luxury but a
\emph{necessity}: operating McEliece with the same rekey interval as
ML-KEM would degrade MAVLink availability below acceptable limits.
...
SoC temperature     & \SI{63.8}{\degreeCelsius} & \SI{62.8}{\degreeCelsius} \\
...
(30~rekeys) consume \SI{0.3}{\joule}---\textbf{0.004\%} of the
flight energy budget.  McEliece-8192128 rekeys consume
\SI{433}{\joule}---\textbf{6.0\%}.  The 1500$\times$ difference
makes ML-KEM the only energetically viable option for frequent
...
ML-KEM-1024 (L5) completes a handshake in \SI{15}{\milli\second};
McEliece-348864 (L1, a \emph{lower} security level) takes
\SI{333}{\milli\second}---22$\times$ longer.  Parameter tuning within
ML-KEM yields at most 1.2$\times$ improvement; switching from
McEliece to ML-KEM at the same level yields 635$\times$.
...
handshake overhead ($>$99.8\% reduction) while maintaining the same
...
The maximum AEAD timing difference is \SI{3.5}{\micro\second}/packet.
At \SI{320}{\hertz}, this is \SI{1.12}{\milli\second\per\second}
---below measurement noise.  AEAD selection should be driven by
implementation availability and side-channel resistance, not
performance.
...
\begin{enumerate}
  ...
  \item \textbf{Rekey interval:} $R = \SI{60}{\second}$ for ML-KEM
    suites ($\PhiR < 0.03\%$).  Increase to $R \geq \SI{300}{\second}$
    for HQC; avoid periodic rekey entirely for McEliece.

  \item \textbf{Avoid in flight:} SPHINCS\textsuperscript{+} (signing
    $> \SI{1}{\second}$) and McEliece-8192128 (keygen
    $> \SI{8}{\second}$).  Keep in registry for ground testing and
    algorithm-diversity auditing.
\end{enumerate}
...
for UAV--GCS communication, backed by \num{19600}~benchmarked
cryptographic operations and 71~successful end-to-end tunnel runs on
a Raspberry~Pi~5.

Our evaluation reveals that the PQC landscape for constrained
platforms is starkly divided: ML-KEM suites achieve
sub-\SI{15}{\milli\second} handshakes with negligible rekey overhead
($\PhiR < 0.03\%$), while McEliece and HQC suites incur
multi-second handshakes that threaten MAVLink continuity.  This
four-order-of-magnitude spread across algorithm families makes
