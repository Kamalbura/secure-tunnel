...
class BenchDroneScheduler:
    def __init__(self):
        ...
        # Telemetry Injection Socket
        self.tx_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self.tx_addr = ("127.0.0.1", DRONE_PLAIN_TX_PORT)
    ...
    def _sensor_fusion_loop(self):
        """10Hz Data Injection."""
        log("Sensor Fusion Loop Active")
        while self.running:
            try:
                # 1. Gather Data
                pwr = self.power_mon.read_metrics() if self.power_mon else {}
                ts = time.time() # Use local valid time
                ...
                packet = {
                    ...
                    "bat_v": pwr.get("bus_v", 0.0),
                    "bat_i": pwr.get("current_mA", 0.0),
                    "seq": int(ts * 10) # Pseudo seq
                }
                
                # 3. Inject to Tunnel
                data = json.dumps(packet).encode()
                self.tx_sock.sendto(data, self.tx_addr)
