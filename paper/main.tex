...
Classic~McEliece tunnels incur up to 0.85\% overhead---a 29$\times$
...
This reduces handshake time by \textbf{98$\times$} (from
\SI{1444}{\milli\second} to \SI{15}{\milli\second}) and public-key
...
McEliece-8192128 is \SI{857}{\milli\second}---reflecting the
high variance of probabilistic key generation.  This unpredictability
...
The mean handshake time increases 2.9$\times$ from L1 to L5, but
this is dominated by the McEliece and SPHINCS\textsuperscript{+}
components.  ML-KEM suites (without SPHINCS\textsuperscript{+}) at
all three levels complete in $< \SI{22}{\milli\second}$.
...
\midrule
ML-KEM-512 + Falcon-512    & L1 & 9.1  & 800 \\
ML-KEM-768 + ML-DSA-65     & L3 & 17.7  & \num{1184} \\
ML-KEM-1024 + Falcon-1024  & L5 & 10.1  & \num{1568} \\
\bottomrule
\end{tabular}
\end{table}
...
handshake is so fast (9--22\,ms) that the SIG choice (Falcon vs.\ ML-DSA)
...
\midrule
ML-KEM-768       & \SI{17.7}{\milli\second}  & 0.029\% & 0.006\% & ${<}0.001\%$ \\
HQC-256          & \SI{272}{\milli\second}   & 0.45\%  & 0.091\% & 0.008\% \\
McE-348864       & \SI{120}{\milli\second}  & 0.20\%  & 0.040\% & 0.003\% \\
McE-8192128      & \SI{517}{\milli\second}        & \textbf{0.85\%} & \textbf{0.17\%} & 0.014\% \\
\bottomrule
\end{tabular}
\end{table}
...
($\PhiR < 0.03\%$), while McEliece-8192128 consumes 0.85\% of the
cycle in handshake---a \textbf{29$\times$} difference.  Combined with
...
(30~rekeys) consume \SI{2.1}{\joule}---\textbf{0.03\%} of the
flight energy budget.  McEliece-8192128 rekeys consume
\SI{62}{\joule}---\textbf{0.86\%}.  The 29$\times$ difference
...
\SI{120}{\milli\second}---11$\times$ longer.  Parameter tuning within
ML-KEM yields at most 2$\times$ improvement; switching from
McEliece to ML-KEM at the same level yields 98$\times$.
...
sub-\SI{22}{\milli\second} handshakes (excluding SPHINCS\textsuperscript{+})
with negligible rekey overhead ($\PhiR < 0.03\%$), while McEliece
